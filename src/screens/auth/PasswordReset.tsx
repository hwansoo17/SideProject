...
const PasswordReset: React.FC<{
  ...
  const { mutate: sendCodeMutate, isPending: isSendCodePending } =
    useSendCode();
  ...
  const handleResetPassword = () => {
    resetPasswordMutate(
      { email: email, newPassword: password, otp: code },
      {
        onSuccess: (data) => {
      ...
      {step == 1 && (
        <View>
          ...
          <CustomButton
            ...
            disabled={code.length != 6 || isVerifyCodePending}
          />
        </View>
      )}
      {step == 2 && (
